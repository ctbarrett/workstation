# Custom version of the default theme for gitprompt.sh
...
define_undefined_git_prompt_colors() {
  ...
  if [ -z ${GIT_PROMPT_START_USER+x} ]; then GIT_PROMPT_START_USER="_LAST_COMMAND_INDICATOR_ ${Red}${UserName}${Cyan}@${Green}${Hostname}${Cyan}:${Yellow}${PathShort}${ResetColor}"; fi
  if [ -z ${GIT_PROMPT_START_ROOT+x} ]; then GIT_PROMPT_START_ROOT="${GIT_PROMPT_START_USER}"; fi
  if [ -z ${GIT_PROMPT_END_USER+x} ]; then GIT_PROMPT_END_USER=" ${Cyan} $ ${ResetColor}"; fi
  if [ -z ${GIT_PROMPT_END_ROOT+x} ]; then GIT_PROMPT_END_ROOT=" ${Cyan} # ${ResetColor}"; fi
